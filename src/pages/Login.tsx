...
export default function Login() {
  ...
  const handleDemoLogin = async () => {
    setIsLoading(true);
    try {
      // Use the mobile mock auth with fixed credentials for instant access
      const { error } = await signInWithMobileMock("9999999999", "1234");
      if (error) {
        toast({ title: "Demo access failed", description: error.message, variant: "destructive" });
      } else {
        toast({ title: "Welcome Demo User", description: "Accessing FinSage Dashboard..." });
        navigate("/");
      }
    } catch (e) {
      toast({ title: "Error", description: "Failed to access demo mode", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
